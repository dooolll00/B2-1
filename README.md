...
먼저 `python3 --version`으로 Python 3.10 이상인지 확인하세요. 기본 `python3`가 3.9이고 Python 3.12가 설치되어 있다면 아래처럼 실행합니다. 이후 예시의 `python`도 `python3.12`로 바꿔 입력하세요.
...
긴 옵션은 **`--`를 사용**합니다. 모든 명령과 하위 명령에 `--help`를 지원합니다.
...
명령별 입력·출력을 함수로 분리하고 검색 조건과 거래 변환을 단계별로 정리했습니다. 기존 명령·저장 방식·데이터 보호 동작을 유지했으며 Python 3.12에서 자동 검사 21개가 모두 통과했습니다. 이번 코드·문서 변경은 아직 GitHub에 반영하지 않았습니다.
